...
export default function styleXVitePlugin({ unstable_moduleResolution = { type: "commonJS", rootDir: process.cwd() }, stylexImports = ["@stylexjs/stylex"], libraries: inputLibraries = [], ...options } = {}) {
    ...
    let modulesToInvalidate = new Set();
    let server;
    let framework = "none";
    let aliases = {};
    let reloadCount = 0;
...
- Installed version of @stylexjs/stylex: ${stylePackageVersion}
    ...
    return {
        ...
        async generateBundle(_, bundle) {
            ...
            if (typeof viteCssPlugin.transform === "function" &&
                typeof viteCssPostPlugin.transform === "function") {
                const cssResult = await viteCssPlugin.transform?.call(this, stylexCSS, "stylex.css");
                const cssResultCode = typeof cssResult === "string"
                    ? cssResult
                    : !!cssResult
                        ? cssResult.code
                        : null;
                if (cssResultCode) {
                    const cssPostResult = await viteCssPostPlugin.transform?.call(this, cssResultCode, "stylex.css?inline-css&used");
                    const cssPostResultCode = typeof cssPostResult === "string"
                        ? cssPostResult
                        : !!cssPostResult
                            ? cssPostResult.code
                            : null;
                    try {
                        const css = cssPostResultCode
                            ? JSON.parse(cssPostResultCode.replace("export default", "").trim()).trim()
                            : null;
                        stylexCSS = css;
                    }
                    catch { }
            ...
            const hashContents = (contents) => crypto.createHash("sha1").update(contents).digest("hex").slice(0, 8);
            if (framework === "remix") {
                const values = Object.values(bundle);
                const cssFilesWithStyleX = values.filter((b) => b.type === "asset" &&
                    b.fileName.endsWith(".css") &&
                    b.source.toString().includes(STYLEX_REPLACE_RULE));
                if (cssFilesWithStyleX.length === 0) {
                    this.warn("Could not find any CSS files with the stylex comment. Did you import styles in the root.tsx of your Remix app?");
                    return;
                }
                for (const cssFile of cssFilesWithStyleX) {
                    let relatedJsChunk = values.find((b) => b.type === "chunk" &&
                        !!b.viteMetadata?.importedCss.has(cssFile.fileName));
                    let shouldReplaceAssetUrl = false;
                    if (!relatedJsChunk) {
                        relatedJsChunk = values.find((b) => b.type === "chunk" && b.code.includes(cssFile.fileName));
                        if (!relatedJsChunk) {
                            this.error(`Could not find related JS chunk for CSS file ${cssFile.fileName}.`);
                        }
                    ...
                    const newCssFileName = path.join(dir, basename.replace(/\-([^.]+)\.css$/, `-${hash}.css`));
                    if (!isSSR) {
                    ...
                    if (shouldReplaceAssetUrl) {
                        const newJsChunkCode = relatedJsChunk.code.replaceAll(cssFile.fileName, newCssFileName);
                        ...
                        const newJsChunkFileName = path.join(dir, basename.replace(/\-([^.]+)\.js$/, `-${hash}.js`));
            ...
            if (framework === "sveltekit") {
                ...
                const cssFilesWithStyleX = values.filter((b) => b.type === "asset" &&
                    ...
                    b.source.toString().includes(STYLEX_REPLACE_RULE));
                if (cssFilesWithStyleX.length === 0) {
                    this.warn("Could not find any CSS files with the stylex comment. Did you import styles in the root of your SvelteKit app?");
                    return;
                }
                for (const cssFile of cssFilesWithStyleX) {
                    const relatedJsChunk = values.find((b) => b.type === "chunk" &&
                        !!b.viteMetadata?.importedCss.has(cssFile.fileName));
                    if (!relatedJsChunk) {
                        this.error(`Could not find related JS chunk for CSS file ${cssFile.fileName}.`);
                        return;
...
`);
        ...
        transformIndexHtml(html, ctx) {
            ...
            const publicPath = path.posix.join(publicBasePath, fileName.replace(/\\/g, "/"));
            return [
                {
                    tag: "link",
                    attrs: {
